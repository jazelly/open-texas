...
export class GameService {
  ...
  async createGame(
    ...
    const texasHoldemGame = new TexasHoldemGame(game.id, name, maxPlayers);
  ...
  async getActiveGames(): Promise<any[]> {
    ...
    return games.map((game) => ({
      ...game,
      players: players.filter((player) => player.gameId === game.id),
    }));
  }
  ...
  async addPlayerToWaitingRoom(gameId: string, userId: string): Promise<any> {
    ...
    console.log("addPlayerToWaitingRoom", userId, user.name, user.chips);
    game.addPlayerToWaitingList(userId, user.name, user.chips);
